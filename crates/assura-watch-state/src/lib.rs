//! Shared filesystem-event invalidation for continuous Assura runtimes.

use notify::event::{AccessKind, AccessMode, CreateKind, EventKind, ModifyKind, RemoveKind};
use notify::Event;
use std::path::{Path, PathBuf};
use std::sync::Mutex;

const MAX_INCREMENTAL_DIRTY_PATHS: usize = 64;

/// Scope that a prepared runtime must revalidate after filesystem changes.
#[derive(Debug, PartialEq, Eq)]
pub enum DirtyProject {
    /// No project paths changed.
    Clean,
    /// The complete project must be validated.
    Full,
    /// Only these paths and their aggregate scopes need validation.
    Paths(Vec<PathBuf>),
}

/// Atomic snapshot removed from [`DirtyState`] before a validation run.
#[derive(Debug)]
pub struct DirtyTake {
    /// Event generation represented by this snapshot.
    pub generation: u64,
    /// Whether the configuration changed.
    pub config_changed: bool,
    /// Project validation scope represented by the snapshot.
    pub project: DirtyProject,
}

/// Thread-safe coalescing state shared by filesystem watcher callbacks.
#[derive(Debug)]
pub struct DirtyState {
    inner: Mutex<DirtySnapshot>,
}

/// Add an explicit path to a coalesced dirty-project scope.
pub fn dirty_project_paths(project: DirtyProject, explicit_path: PathBuf) -> Option<Vec<PathBuf>> {
    match project {
        DirtyProject::Clean => Some(vec![explicit_path]),
        DirtyProject::Paths(mut paths) => {
            if !paths_match_any(&paths, &explicit_path) {
                paths.push(explicit_path);
            }
            Some(paths)
        }
        DirtyProject::Full => None,
    }
}

fn paths_match_any(paths: &[PathBuf], path: &Path) -> bool {
    paths
        .iter()
        .any(|existing| paths_refer_to_same_change(existing, path))
}

fn paths_refer_to_same_change(left: &Path, right: &Path) -> bool {
    left == right || left.ends_with(right) || right.ends_with(left)
}

#[derive(Debug)]
struct DirtySnapshot {
    generation: u64,
    config_changed: bool,
    project_changed: bool,
    full_project: bool,
    paths: Vec<PathBuf>,
}

impl DirtyState {
    /// Create state that requires one initial full-project validation.
    pub fn new() -> Self {
        Self {
            inner: Mutex::new(DirtySnapshot {
                generation: 0,
                config_changed: false,
                project_changed: true,
                full_project: true,
                paths: Vec::new(),
            }),
        }
    }

    /// Record one watcher event and return whether it invalidated validation state.
    pub fn record_event(&self, event: &Event, config_path: &Path) -> bool {
        self.record_event_with_publication(event, config_path, || ())
            .is_some()
    }

    /// Record one watcher event and publish its invalidation before another
    /// thread can consume the new generation.
    pub fn record_event_with_publication<R>(
        &self,
        event: &Event,
        config_path: &Path,
        publish: impl FnOnce() -> R,
    ) -> Option<R> {
        let event = classify_event(event, config_path);
        if matches!(event, DirtyEvent::Ignore) {
            return None;
        }

        let mut snapshot = self.inner.lock().expect("dirty state poisoned");
        apply_event(&mut snapshot, event);
        Some(publish())
    }

    /// Atomically remove and return pending invalidation state.
    pub fn take(&self) -> DirtyTake {
        let mut snapshot = self.inner.lock().expect("dirty state poisoned");
        let generation = snapshot.generation;
        let config_changed = std::mem::take(&mut snapshot.config_changed);
        let project = if !snapshot.project_changed {
            DirtyProject::Clean
        } else if snapshot.full_project || config_changed || snapshot.paths.is_empty() {
            DirtyProject::Full
        } else {
            DirtyProject::Paths(std::mem::take(&mut snapshot.paths))
        };
        snapshot.project_changed = false;
        snapshot.full_project = false;
        snapshot.paths.clear();
        DirtyTake {
            generation,
            config_changed,
            project,
        }
    }

    /// Return whether a configuration event is pending.
    pub fn config_changed(&self) -> bool {
        self.inner
            .lock()
            .expect("dirty state poisoned")
            .config_changed
    }

    /// Return whether an invalidating event arrived after a taken snapshot.
    pub fn changed_since(&self, generation: u64) -> bool {
        self.inner.lock().expect("dirty state poisoned").generation > generation
    }

    /// Observe whether the state changed since `generation` while preventing a
    /// watcher callback from recording a newer event until `publish` returns.
    ///
    /// Status-file writers use this to avoid publishing a stale clean result:
    /// either the newer event is included in `changed`, or it is recorded after
    /// the clean publication and the watcher publishes dirty state next.
    pub fn with_change_state_since<R>(
        &self,
        generation: u64,
        publish: impl FnOnce(bool) -> R,
    ) -> R {
        let snapshot = self.inner.lock().expect("dirty state poisoned");
        publish(snapshot.generation > generation)
    }

    /// Run a status publication while watcher generations are held stable.
    pub fn serialize_publication<R>(&self, publish: impl FnOnce() -> R) -> R {
        let _snapshot = self.inner.lock().expect("dirty state poisoned");
        publish()
    }
}

fn apply_event(snapshot: &mut DirtySnapshot, event: DirtyEvent) {
    snapshot.generation = snapshot.generation.wrapping_add(1);
    snapshot.project_changed = true;
    match event {
        DirtyEvent::Ignore => unreachable!("ignored events return before locking"),
        DirtyEvent::Config => {
            snapshot.config_changed = true;
            snapshot.full_project = true;
            snapshot.paths.clear();
        }
        DirtyEvent::Full => {
            snapshot.full_project = true;
            snapshot.paths.clear();
        }
        DirtyEvent::Paths(paths) if !snapshot.full_project => {
            for path in paths {
                if snapshot.paths.iter().any(|existing| existing == &path) {
                    continue;
                }
                snapshot.paths.push(path);
                if snapshot.paths.len() > MAX_INCREMENTAL_DIRTY_PATHS {
                    snapshot.full_project = true;
                    snapshot.paths.clear();
                    return;
                }
            }
        }
        DirtyEvent::Paths(_) => {}
    }
}

impl Default for DirtyState {
    fn default() -> Self {
        Self::new()
    }
}

enum DirtyEvent {
    Ignore,
    Config,
    Full,
    Paths(Vec<PathBuf>),
}

fn classify_event(event: &Event, config_path: &Path) -> DirtyEvent {
    if event.need_rescan() {
        return DirtyEvent::Full;
    }
    if event.paths.iter().any(|path| path == config_path) {
        return DirtyEvent::Config;
    }

    match event.kind {
        EventKind::Access(AccessKind::Close(AccessMode::Write)) => {
            existing_file_paths_or_full(event)
        }
        EventKind::Access(_) => DirtyEvent::Ignore,
        EventKind::Create(CreateKind::File) | EventKind::Remove(RemoveKind::File) => {
            event_paths_or_full(event)
        }
        EventKind::Create(CreateKind::Any | CreateKind::Other)
        | EventKind::Modify(ModifyKind::Any | ModifyKind::Other)
        | EventKind::Modify(ModifyKind::Data(_) | ModifyKind::Metadata(_))
        | EventKind::Modify(ModifyKind::Name(_)) => existing_file_paths_or_full(event),
        EventKind::Create(CreateKind::Folder)
        | EventKind::Remove(_)
        | EventKind::Any
        | EventKind::Other => DirtyEvent::Full,
    }
}

fn event_paths_or_full(event: &Event) -> DirtyEvent {
    if event.paths.is_empty() {
        DirtyEvent::Full
    } else {
        DirtyEvent::Paths(event.paths.clone())
    }
}

fn existing_file_paths_or_full(event: &Event) -> DirtyEvent {
    if event.paths.is_empty() {
        return DirtyEvent::Full;
    }

    let mut saw_file = false;
    for path in &event.paths {
        if path.is_dir() {
            return DirtyEvent::Full;
        }
        if path.is_file()
            || path
                .symlink_metadata()
                .is_ok_and(|metadata| metadata.is_symlink())
        {
            saw_file = true;
        }
    }

    if saw_file {
        DirtyEvent::Paths(event.paths.clone())
    } else {
        DirtyEvent::Full
    }
}

#[cfg(test)]
mod concurrency_tests;

#[cfg(test)]
mod tests {
    use super::*;
    use notify::event::DataChange;

    #[test]
    fn access_events_do_not_dirty_project() {
        let temp = tempfile::tempdir().unwrap();
        let state = DirtyState::new();
        state.take();
        let recorded = state.record_event(
            &Event::new(EventKind::Access(AccessKind::Read))
                .add_path(temp.path().join("src").join("file.ts")),
            &temp.path().join(".assura/config.yml"),
        );

        assert!(!recorded);
        assert_eq!(state.take().project, DirtyProject::Clean);
    }

    #[test]
    fn event_after_take_remains_observably_dirty() {
        let temp = tempfile::tempdir().unwrap();
        let state = DirtyState::new();
        let taken = state.take();

        state.record_event(
            &Event::new(EventKind::Create(CreateKind::File)).add_path(temp.path().join("late.ts")),
            &temp.path().join(".assura/config.yml"),
        );

        assert!(state.changed_since(taken.generation));
        assert_eq!(
            state.take().project,
            DirtyProject::Paths(vec![temp.path().join("late.ts")])
        );
    }

    #[test]
    fn write_close_access_tracks_incremental_path() {
        let temp = tempfile::tempdir().unwrap();
        let path = temp.path().join("file.ts");
        std::fs::write(&path, "").unwrap();
        let state = DirtyState::new();
        state.take();
        state.record_event(
            &Event::new(EventKind::Access(AccessKind::Close(AccessMode::Write)))
                .add_path(path.clone()),
            &temp.path().join(".assura/config.yml"),
        );

        assert_eq!(state.take().project, DirtyProject::Paths(vec![path]));
    }

    #[test]
    fn file_create_tracks_incremental_path() {
        let temp = tempfile::tempdir().unwrap();
        let path = temp.path().join("src").join("file.ts");
        let state = DirtyState::new();
        state.take();
        state.record_event(
            &Event::new(EventKind::Create(CreateKind::File)).add_path(path.clone()),
            &temp.path().join(".assura/config.yml"),
        );

        assert_eq!(state.take().project, DirtyProject::Paths(vec![path]));
    }

    #[test]
    fn file_burst_coalesces_distinct_paths_and_generations() {
        let temp = tempfile::tempdir().unwrap();
        let state = DirtyState::new();
        state.take();
        let paths = ["one.ts", "two.ts", "three.ts"]
            .map(|name| temp.path().join(name))
            .to_vec();

        for path in &paths {
            state.record_event(
                &Event::new(EventKind::Create(CreateKind::File)).add_path(path.clone()),
                &temp.path().join(".assura/config.yml"),
            );
        }

        let dirty = state.take();
        assert_eq!(dirty.generation, 3);
        assert_eq!(dirty.project, DirtyProject::Paths(paths));
    }

    #[test]
    fn folder_create_requires_full_project_check() {
        let temp = tempfile::tempdir().unwrap();
        let state = DirtyState::new();
        state.take();
        state.record_event(
            &Event::new(EventKind::Create(CreateKind::Folder)).add_path(temp.path().join("src")),
            &temp.path().join(".assura/config.yml"),
        );

        assert_eq!(state.take().project, DirtyProject::Full);
    }

    #[test]
    fn config_changes_request_config_reload_and_full_check() {
        let temp = tempfile::tempdir().unwrap();
        let config = temp.path().join(".assura/config.yml");
        let state = DirtyState::new();
        state.take();
        state.record_event(
            &Event::new(EventKind::Modify(ModifyKind::Data(DataChange::Content)))
                .add_path(config.clone()),
            &config,
        );

        let dirty = state.take();
        assert!(dirty.config_changed);
        assert_eq!(dirty.project, DirtyProject::Full);
    }

    #[test]
    fn config_changed_check_does_not_clear_project_dirty_state() {
        let temp = tempfile::tempdir().unwrap();
        let config = temp.path().join(".assura/config.yml");
        let state = DirtyState::new();
        state.take();
        state.record_event(
            &Event::new(EventKind::Modify(ModifyKind::Data(DataChange::Content)))
                .add_path(config.clone()),
            &config,
        );

        assert!(state.config_changed());
        assert!(state.config_changed());
        let dirty = state.take();
        assert!(dirty.config_changed);
        assert_eq!(dirty.project, DirtyProject::Full);
    }

    #[test]
    fn dirty_project_paths_uses_explicit_path_when_clean() {
        assert_eq!(
            dirty_project_paths(DirtyProject::Clean, PathBuf::from("src/file.ts")),
            Some(vec![PathBuf::from("src/file.ts")])
        );
    }

    #[test]
    fn dirty_project_paths_preserves_pending_watcher_paths() {
        assert_eq!(
            dirty_project_paths(
                DirtyProject::Paths(vec![PathBuf::from("src/other.ts")]),
                PathBuf::from("src/file.ts"),
            ),
            Some(vec![
                PathBuf::from("src/other.ts"),
                PathBuf::from("src/file.ts")
            ])
        );
    }

    #[test]
    fn dirty_project_paths_deduplicates_explicit_relative_path() {
        assert_eq!(
            dirty_project_paths(
                DirtyProject::Paths(vec![PathBuf::from("/project/src/file.ts")]),
                PathBuf::from("src/file.ts"),
            ),
            Some(vec![PathBuf::from("/project/src/file.ts")])
        );
    }

    #[test]
    fn dirty_project_paths_deduplicates_explicit_absolute_path() {
        assert_eq!(
            dirty_project_paths(
                DirtyProject::Paths(vec![PathBuf::from("src/file.ts")]),
                PathBuf::from("/project/src/file.ts"),
            ),
            Some(vec![PathBuf::from("src/file.ts")])
        );
    }

    #[test]
    fn dirty_project_paths_rejects_full_dirty_state() {
        assert_eq!(
            dirty_project_paths(DirtyProject::Full, PathBuf::from("src/file.ts")),
            None
        );
    }
}
