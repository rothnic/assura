...
The public performance summary should prefer `assura-check-cli` and
`ls-lint-cli` rows from the same release profile. The current checked-in
headline claim is scoped to the Linux static-CRT `assura-check-cli` artifact;
local macOS dynamic rows are diagnostic lower-bound evidence. The older
`assura-cli` row remains diagnostic for measuring the full multi-command
binary.
...
| --- | --- | --- |
| `assura-check-cli` | Lightweight Assura check-only subprocess | Product comparison against LS-Lint |
| `assura-cli` | Full Assura multi-command subprocess check | Startup/runtime attribution |
...
when every measured `realistic-equivalent` `assura-check-cli` row has a matching
...
Lower-sample smoke reports use `not-complete-low-sample` instead of `complete`.
Diagnostic rows such as hot daemon checks, status-file checks, in-process
validation, and traversal experiments are excluded from that verdict.
