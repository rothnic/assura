...
current checked-in headline claim is scoped to the Linux static-CRT focused
check artifact behind that path; local macOS dynamic rows are diagnostic
lower-bound evidence. The older `assura-cli` row remains diagnostic for
measuring full multi-command CLI startup overhead.
...
| --- | --- | --- |
| `assura-check-cli` | Focused checker used by the primary `assura check` path | Product comparison against LS-Lint |
| `assura-cli` | Full Assura multi-command subprocess check | Historical startup/runtime attribution |
