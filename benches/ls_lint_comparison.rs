...
use assura::constraints::{
    ...
    DirectoryConstraint, DirectoryValidationConfig, NamingConstraint, FileSizeConstraint, FileSizeRule, FileSizeLimit,
};
...
fn create_rust_project(module_count: usize) -> TempDir {
    let temp_dir = Builder::new().prefix("assura_rust_bench_").tempdir().unwrap();
    let base_path = temp_dir.path();
    ...
    let size = FileSizeConstraint::new()
        .add_rule(
            FileSizeRule::new("max_size")
                .max_size(FileSizeLimit::Megabytes(1))
        );
...
fn run_ls_lint(dir: &Path) -> Option<Duration> {
    ...
    let start = Instant::now();
    let result = Command::new("ls-lint")
        .current_dir(dir)
        .output();
...
fn bench_full_validation(c: &mut Criterion) {
    ...
    for (name, dirs, files_per) in scenarios {
        ...
        group.bench_with_input(
            BenchmarkId::new("assura", name),
            &file_count,
            |b, _| {
                b.iter(|| {
                    black_box(validate_directory_assura(&engine, temp_dir.path()))
                })
            },
        );
        ...
        if ls_lint_available() {
            group.bench_with_input(
                BenchmarkId::new("ls_lint", name),
                &file_count,
                |b, _| {
                    b.iter(|| {
                        black_box(run_ls_lint(temp_dir.path()))
                    })
                },
            );
        }
...
fn bench_project_types(c: &mut Criterion) {
    ...
    group.bench_function("assura_rust", |b| {
        b.iter(|| {
            black_box(validate_directory_assura(&rust_engine, rust_dir.path()))
        })
    });
    ...
    if ls_lint_available() {
        group.bench_function("ls_lint_rust", |b| {
            b.iter(|| {
                black_box(run_ls_lint(rust_dir.path()))
            })
        });
    }
    ...
    group.bench_function("assura_js", |b| {
        b.iter(|| {
            black_box(validate_directory_assura(&js_engine, js_dir.path()))
        })
    });
    ...
    if ls_lint_available() {
        group.bench_function("ls_lint_js", |b| {
            b.iter(|| {
                black_box(run_ls_lint(js_dir.path()))
            })
        });
    }
...
fn bench_incremental_validation(c: &mut Criterion) {
    ...
    group.bench_function("assura_single_file", |b| {
        b.iter(|| {
            black_box(engine.validate(&test_file, &context))
        })
    });
...
fn bench_complex_rules(c: &mut Criterion) {
    ...
    group.bench_function("assura_4_rules", |b| {
        b.iter(|| {
            black_box(validate_directory_assura(&complex_engine, temp_dir.path()))
        })
    });
