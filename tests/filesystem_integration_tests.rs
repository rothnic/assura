...
fn create_test_project() -> TempDir {
    ...
    let mut card_file = File::create(base_path.join("src/components/Card.tsx"))
        .expect("Should create Card.tsx");
...
fn create_bad_naming_project() -> TempDir {
    ...
    let mut main_file = File::create(base_path.join("src/MainFile.rs")).expect("Should create MainFile.rs");
    writeln!(main_file, "fn main() {{}}").expect("Should write");

    let mut lib_file = File::create(base_path.join("src/my-lib.rs")).expect("Should create my-lib.rs");
    writeln!(lib_file, "pub fn add() {{}}").expect("Should write");
...
fn test_validate_correct_naming() {
    use assura::constraints::{NamingConstraint, CaseConvention, Constraint};
    ...
    let result = constraint.validate(Path::new(base_path.join("src/main.rs").to_str().unwrap()), &context)
        .expect("Should validate");
    assert!(result.passed, "main.rs should pass snake_case validation");

    let result = constraint.validate(Path::new(base_path.join("src/lib.rs").to_str().unwrap()), &context)
        .expect("Should validate");
    ...
    let result = constraint.validate(Path::new(base_path.join("src/components/Button.tsx").to_str().unwrap()), &context)
        .expect("Should validate");
    assert!(result.passed, "Button.tsx should pass PascalCase validation");

    let result = constraint.validate(Path::new(base_path.join("src/components/Card.tsx").to_str().unwrap()), &context)
        .expect("Should validate");
...
fn test_validate_incorrect_naming() {
    use assura::constraints::{NamingConstraint, CaseConvention};
    use assura::constraints::ConstraintContext;
    use std::path::Path;
    ...
    let result = constraint.validate(Path::new(base_path.join("src/MainFile.rs").to_str().unwrap()), &context)
        .expect("Should validate");
    assert!(!result.passed, "MainFile.rs should fail snake_case validation");

    let result = constraint.validate(Path::new(base_path.join("src/my-lib.rs").to_str().unwrap()), &context)
        .expect("Should validate");
    assert!(!result.passed, "my-lib.rs should fail snake_case validation (kebab-case not snake_case)");
    ...
    let result = constraint.validate(Path::new(base_path.join("src/components/button.tsx").to_str().unwrap()), &context)
        .expect("Should validate");
    assert!(!result.passed, "button.tsx should fail PascalCase validation");

    let result = constraint.validate(Path::new(base_path.join("src/components/my_card.tsx").to_str().unwrap()), &context)
        .expect("Should validate");
    assert!(!result.passed, "my_card.tsx should fail PascalCase validation");
}

#[test]
fn test_file_size_validation() {
    use assura::constraints::{FileSizeConstraint, FileSizeRule, FileSizeLimit, Severity};
    use assura::constraints::ConstraintContext;
    use std::path::Path;
    ...
    small_file.write_all(b"small content").expect("Should write");

    // Create a larger file
    let mut large_file = File::create(base_path.join("large.txt")).expect("Should create file");
    large_file.write_all(&vec![0u8; 2000]).expect("Should write"); // 2KB
    ...
    let result = constraint.validate(Path::new(base_path.join("small.txt").to_str().unwrap()), &context)
        .expect("Should validate");
    assert!(result.passed, "Small file should pass size constraint");

    // Large file should fail
    let result = constraint.validate(Path::new(base_path.join("large.txt").to_str().unwrap()), &context)
        .expect("Should validate");
    ...
    for entry in entries {
        ...
        if path.extension().map(|e| e == "rs").unwrap_or(false) {
            ...
            let result = constraint.validate(path, &context)
                .expect("Should validate");
            assert!(result.passed, "All Rust files should pass snake_case: {:?}", path);
        }
    }
...
fn test_multi_part_extensions_real_files() {
    ...
    let mut test_file = File::create(base_path.join("src/my_component.tsx"))
        .expect("Should create test file");
    writeln!(test_file, "export function MyComponent() {{}}").expect("Should write");

    let mut spec_file = File::create(base_path.join("src/my_component.ts"))
        .expect("Should create spec file");
    writeln!(spec_file, "export function myComponent() {{}}").expect("Should write");

    // Invalid naming (PascalCase instead of snake_case)
    let mut bad_file = File::create(base_path.join("src/MyComponent.tsx"))
        .expect("Should create bad file");
    ...
    let result = constraint.validate(Path::new(base_path.join("src/my_component.tsx").to_str().unwrap()), &context)
        .expect("Should validate");
    assert!(result.passed, "snake_case file should pass");

    let result = constraint.validate(Path::new(base_path.join("src/my_component.ts").to_str().unwrap()), &context)
        .expect("Should validate");
    assert!(result.passed, "snake_case file should pass");

    // Invalid file should fail
    let result = constraint.validate(Path::new(base_path.join("src/MyComponent.tsx").to_str().unwrap()), &context)
        .expect("Should validate");
