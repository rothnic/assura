...
fn test_ls_lint_directory_exclusions() {
    let config = DirectoryValidationConfig::new().with_excluded_dir("custom_build");
...
fn test_ls_lint_multi_part_extension_d_ts() {
    ...
    let rule = MultiPartExtensionRule::new().allow_extension("d.ts");
...
fn test_ls_lint_nested_path_rules() {
    ...
    let child_rule =
        parent.find_matching_rule(PathBuf::from("src/components/MyComponent.rs").as_path());
