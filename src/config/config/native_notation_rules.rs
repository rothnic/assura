...
impl RuleRegistry {
    pub(super) fn from_root(root: &mut Mapping) -> Result<Self, String> {
    ...
    pub(super) fn resolve_node(&self, reference: &str) -> Result<Value, String> {
    ...
    pub(super) fn resolve_tree(&self, reference: &str) -> Result<Mapping, String> {
        let name = parse_rule_reference(reference)?;
        let value = self.resolve(name)?;
        match classify_fragment(&value)? {
            FragmentKind::Tree => {
...
pub(super) fn classify_fragment(value: &Value) -> Result<FragmentKind, String> {
    let Value::Mapping(mapping) = value else {
...
pub(super) fn parse_rule_reference(reference: &str) -> Result<&str, String> {
    reference
        .strip_prefix('@')
...
pub(super) fn is_rule_reference(value: &str) -> bool {
    value.starts_with('@')
}
