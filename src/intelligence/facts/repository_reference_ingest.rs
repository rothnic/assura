...
impl FactIngestor {
    ...
    pub fn ingest_source_references(
        ...
        for reference in references {
            self.upsert_repository_reference_edge(project_root, source_rel, &reference, &source_id);
        }
    }

    /// Ingest configured Markdown frontmatter repository references.
    pub fn ingest_frontmatter_references(
        &mut self,
        project_root: &Path,
        source_rel: &Path,
        content: &str,
        ...
        let references = frontmatter_references(source_rel, content, fields);
        if references.is_empty() {
            return;
        }
        let source_id = self.upsert_repository_reference_resource(source_rel);
    ...
    fn upsert_repository_reference_edge(
        &mut self,
        project_root: &Path,
        source_rel: &Path,
