        ...
        if pattern.contains("${") {
            ...
            if parts.len() == 2 {
                ...
                if file_name.starts_with(prefix) && file_name.ends_with(suffix) {
                    ...
                    return !middle.is_empty() && middle.chars().all(|c| c.is_alphanumeric() || c == '_');
                }
            }
            return false;
        }
...
mod tests {
    use super::*;
    use crate::config::ast::{PolicyNode, PolicyEntry};
    use std::collections::HashMap;
    ...
    fn test_resolve_simple_rule() {
        ...
        let result = RuleResolver::resolve(
            &config,
            Path::new("/src/Button.tsx"),
        );
