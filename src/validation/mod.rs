...
pub use pairing::{PairingValidator, PairingRequirement, PairingViolation};
pub use resolver::{RuleResolver, ResolvedConstraints};
...
impl ValidationEngine {
    ...
    pub fn validate_file(
        ...
        for constraint in &resolved.constraints {
            let mut result = ConstraintValidator::validate(
                constraint,
                file_path,
                file_content,
            );
    ...
    pub fn should_block(&self,
        ...
        results.iter().any(|r| {
            !r.passed && !allowed_levels.iter().any(|allowed| {
...
mod tests {
    use super::*;
    use crate::config::ast::{Config, PolicyNode, Rule, NamingConvention};
...
"#;
        ...
        let results = engine.validate_file(
            Path::new("src/components/my_component.tsx"),
            None,
        );
...
"#;
        ...
        let should_block = engine.should_block(&results, &[ViolationLevel::Info, ViolationLevel::Warn]);
...
"#;
        ...
        let should_block = engine.should_block(&results, &[ViolationLevel::Warn, ViolationLevel::Block]);
...
"#;
        ...
        assert!(!results[0].passed, "Should fail for file exceeding line limit");
    }
}
