...
impl ConstraintValidator {
    ...
    pub fn validate(
        ...
        match constraint {
            Constraint::Naming(convention) => {
                Self::validate_naming(convention, file_path)
            }
            ...
            Constraint::Size { size } => {
                Self::validate_size(size, file_path)
            }
    ...
    fn validate_naming(
        ...
        let is_valid = match convention {
            ...
            NamingConvention::ScreamingSnakeCase => {
                Self::is_screaming_snake_case(file_name)
            }
    ...
    fn check_range(value: u64, range_str: &str) -> bool {
        if range_str.contains("..") {
            ...
            range_str.parse::<u64>().map_or(false, |expected| value == expected)
        }
    }
...
mod tests {
    ...
    fn test_screaming_snake_case_comprehensive() {
        ...
        assert!(ConstraintValidator::is_screaming_snake_case("MY_LONG_COMPONENT_NAME"));
        ...
        assert!(ConstraintValidator::is_screaming_snake_case("COMPONENT_1_V2")); // Multiple numbers
        assert!(ConstraintValidator::is_screaming_snake_case("MY_1_COMPONENT")); // Number in middle
        ...
        assert!(ConstraintValidator::is_screaming_snake_case("HTTP_STATUS_CODE"));
        ...
        assert!(!ConstraintValidator::is_screaming_snake_case("MY__COMPONENT")); // Consecutive underscores
        assert!(!ConstraintValidator::is_screaming_snake_case("MY_COMPONENT_")); // Trailing underscore
        assert!(!ConstraintValidator::is_screaming_snake_case("_MY_COMPONENT")); // Leading underscore
        assert!(!ConstraintValidator::is_screaming_snake_case("my_component")); // Lowercase
        assert!(!ConstraintValidator::is_screaming_snake_case("MyComponent")); // Mixed case
        assert!(!ConstraintValidator::is_screaming_snake_case("MY-COMPONENT")); // Hyphen
        assert!(!ConstraintValidator::is_screaming_snake_case("MY COMPONENT")); // Space
        assert!(!ConstraintValidator::is_screaming_snake_case("MY@COMPONENT")); // Special char
    }

    #[test]
    fn test_lowercase_comprehensive() {
        // Valid cases
        assert!(ConstraintValidator::validate_naming(&NamingConvention::Lowercase, Path::new("a.rs")).passed);
        assert!(ConstraintValidator::validate_naming(&NamingConvention::Lowercase, Path::new("button.rs")).passed);
        assert!(ConstraintValidator::validate_naming(&NamingConvention::Lowercase, Path::new("mycomponent.rs")).passed);
        assert!(ConstraintValidator::validate_naming(&NamingConvention::Lowercase, Path::new("component1.rs")).passed);
        assert!(ConstraintValidator::validate_naming(&NamingConvention::Lowercase, Path::new("a1b2c3.rs")).passed);

        // Invalid cases
        assert!(!ConstraintValidator::validate_naming(&NamingConvention::Lowercase, Path::new("")).passed); // Empty
        assert!(!ConstraintValidator::validate_naming(&NamingConvention::Lowercase, Path::new("Button.rs")).passed); // Uppercase
        assert!(!ConstraintValidator::validate_naming(&NamingConvention::Lowercase, Path::new("myComponent.rs")).passed); // Mixed case
        assert!(!ConstraintValidator::validate_naming(&NamingConvention::Lowercase, Path::new("BUTTON.rs")).passed); // All uppercase
        assert!(!ConstraintValidator::validate_naming(&NamingConvention::Lowercase, Path::new("my_component.rs")).passed); // Underscore
        assert!(!ConstraintValidator::validate_naming(&NamingConvention::Lowercase, Path::new("my-component.rs")).passed); // Hyphen
    }

    #[test]
    fn test_uppercase_comprehensive() {
        // Valid cases
        assert!(ConstraintValidator::validate_naming(&NamingConvention::Uppercase, Path::new("A.rs")).passed);
        assert!(ConstraintValidator::validate_naming(&NamingConvention::Uppercase, Path::new("BUTTON.rs")).passed);
        assert!(ConstraintValidator::validate_naming(&NamingConvention::Uppercase, Path::new("MYCOMPONENT.rs")).passed);
        assert!(ConstraintValidator::validate_naming(&NamingConvention::Uppercase, Path::new("COMPONENT1.rs")).passed);
        assert!(ConstraintValidator::validate_naming(&NamingConvention::Uppercase, Path::new("A1B2C3.rs")).passed);

        // Invalid cases
        assert!(!ConstraintValidator::validate_naming(&NamingConvention::Uppercase, Path::new("")).passed); // Empty
        assert!(!ConstraintValidator::validate_naming(&NamingConvention::Uppercase, Path::new("button.rs")).passed); // Lowercase
        assert!(!ConstraintValidator::validate_naming(&NamingConvention::Uppercase, Path::new("myComponent.rs")).passed); // Mixed case
        assert!(!ConstraintValidator::validate_naming(&NamingConvention::Uppercase, Path::new("my_component.rs")).passed); // Underscore
        assert!(!ConstraintValidator::validate_naming(&NamingConvention::Uppercase, Path::new("MY-COMPONENT.rs")).passed); // Hyphen
    }
    ...
    fn test_naming_with_multi_part_extensions() {
        ...
        assert!(ConstraintValidator::validate_naming(&NamingConvention::PascalCase, Path::new("MyComponent.ts")).passed);
        assert!(ConstraintValidator::validate_naming(&NamingConvention::SnakeCase, Path::new("my_component.js")).passed);
        assert!(ConstraintValidator::validate_naming(&NamingConvention::KebabCase, Path::new("my-component.rs")).passed);
        assert!(ConstraintValidator::validate_naming(&NamingConvention::CamelCase, Path::new("myComponent.go")).passed);
    }
    ...
    fn test_size_parsing() {
        assert_eq!(ConstraintValidator::parse_size("1MB"), (1, "MB".to_string()));
        assert_eq!(ConstraintValidator::parse_size("500KB"), (500, "KB".to_string()));
        assert_eq!(ConstraintValidator::parse_size("100"), (100, "B".to_string()));
    }
}
