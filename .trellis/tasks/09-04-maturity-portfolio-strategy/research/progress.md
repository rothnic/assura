...
- The fresh 129a249 candidate identity and two source-only canaries pass all seven public dimensions with a meaningful negative control; they are no-credit preparation. The private binding/manifest rereview found `A07-129-HOLDOUT-PROVENANCE-001`, which was corrected by rebinding all six handles to current-candidate construction metadata while preserving immutable creation timestamps and historical source digests; scoped protocol rereview then returned PASS. Independent process review found `A07-RESUME-ROUTE-001` in duplicate current summaries; those routes were corrected and no allocation or credit is granted.
