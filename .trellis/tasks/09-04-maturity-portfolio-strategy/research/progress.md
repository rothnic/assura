...
- The fresh 129a249 candidate identity and two source-only canaries pass all seven public dimensions with a meaningful negative control; they are no-credit preparation. The private binding/manifest rereview found `A07-129-HOLDOUT-PROVENANCE-001`, which was corrected by rebinding all six handles to current-candidate construction metadata while preserving immutable creation timestamps and historical source digests; scoped protocol rereview then returned PASS. The prior ebed packet remains historical and no allocation or credit is granted.
- The ledger remains 32/0/5/3 with A07 active and R01/W02/F01 held. Next owner/action: refresh source, release/tag, PR/CI and topology state before the separately authorized screening gate; context level: not exposed.
