...
| 2026-07-01 | Continued the daemon child after PR #116 merged because the broader goal still required reference-query parity over daemon IPC. The follow-up slice adds `assura daemon references` IPC for source, target, and moved-target queries, keeps one-shot fallback, and proves stale config returns structured daemon errors instead of fresh reference results. | `src/cli/daemon_process.rs`; `src/cli/daemon.rs`; `tests/daemon_reference_cli_tests.rs`; `.trellis/spec/assura/daemon-management-cli.md`; `cargo test --test daemon_reference_cli_tests -- --test-threads=1`; `cargo test --test daemon_core_tests --quiet`. |
| 2026-07-01 | Proved the follow-up daemon IPC path on a release bundle with a 3000-file Markdown fixture. Managed daemon startup now defaults to local loopback TCP to avoid macOS Unix socket path limits, warm `daemon check-path` averaged 12.5 ms versus 2249.2 ms cold, and warm `daemon references --target` averaged 100.4 ms versus 2409.4 ms cold after the review-found stale-source refresh fix. | `src/cli/daemon_process.rs`; `target/release/assura`; `target/release/assura-full`; `target/performance/daemon-reference-ipc-local.json`; `hyperfine --warmup 1 --runs 5 ...`; long-path daemon start/stop smoke; review agent `019f212e-4928-76a0-a351-694f3dd4c279`. |
| 2026-07-02 | Completed true daemon mode via PR #117. The merged slice provides a real managed local process, versioned health/check/reference IPC, stale config and stale-source safety, moved-target degraded context, one-shot fallback, local loopback TCP default, and release/CI proof across supported platforms. The post-beta parent should continue with the Markdown engine child goal. | PR #117; merge commit `745455215d757e49fb4614a170e48f046cb829ad`; `.trellis/tasks/archive/2026-07/07-01-07-01-true-daemon-mode/prd.md`; GitHub checks for PR #117. |
