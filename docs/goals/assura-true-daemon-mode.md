...
| 2026-07-01 | Kept Windows daemon coverage to compile, installer, and installable-adoption smoke while marking managed subprocess lifecycle CLI regressions as Unix CI coverage. Windows full-suite runs repeatedly hung after daemon subprocess tests even though Windows install smoke passed, so Windows-specific daemon lifecycle hardening remains a follow-up instead of blocking this Unix-proven true-daemon slice. | `tests/daemon_cli_tests.rs`; PR #116 checks; `Windows Installer Smoke`; `Installable Adoption Smoke (windows-x86_64)`. |
