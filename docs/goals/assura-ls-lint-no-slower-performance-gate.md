...
`cargo xtask performance-no-slower` is intentionally expected to fail whenever
any headline realistic-equivalent fixture has an `assura-cli` median greater
than its paired native `ls-lint-cli` median. Use
...
| 2026-07-01 | Closed the beta no-slower gate after the final roadmap audit and reviewer-requested fresh release evidence. The regenerated clean report at commit `244f1e5` shows cold `assura-cli` no slower than native `ls-lint-cli` on all 8 headline realistic-equivalent fixtures, while the stricter cold 2x claim remains explicitly `not-complete`; warm daemon/session evidence remains separate and complete. | Independent review Feynman; `cargo build --release --bin assura --no-default-features --features json-output,yaml-config`; `cargo build --release --bin assura-full`; `cargo build --release -p assura-check-cli`; `target/release/assura performance-report --output benches/history/current.json --history benches/history/ls-lint-comparison-history.jsonl --website-dir website/public/data/performance --iterations 5`; `cargo xtask performance-no-slower`; `jq '.commit_sha,.source_worktree_dirty,.claim_summary,.warm_claim_summary,.ls_lint_status' benches/history/current.json`. |
