...
| 2026-07-05 | Reworked `assura review --format text` into a concise aligned human CLI report with terminal-aware ANSI color. JSON and agent output remain unchanged. Captured/piped text stays plain by default, while `ASSURA_FORCE_COLOR=1` enables deterministic color for screenshots or demos. The near-limit report module was split naturally by moving human text rendering into `src/cli/project_review/text.rs`. | `src/cli/project_review/text.rs`; `src/cli/project_review/report.rs`; `tests/project_review_cli.rs`; `.trellis/spec/assura/compact-project-review.md`; `docs/validation.md`; `website/src/content/docs/reference/api.md`; `cargo test --test project_review_cli --quiet` now runs 8 tests; `ASSURA_FORCE_COLOR=1 cargo run --quiet -- review . --format text`. |
