...
projects. It should outrank narrow project-type specialization and lower-level
...
These are more universal than project-specific scoring and support the top-down
...
models, source-document custody, and traceability.
...
- The document-project preset, requirements/evidence traceability, and computed
  checks are captured as separately executable goals.
- Document-heavy behavior remains generic rather than bloating the core preset
  with one customer's workflow.
...
| 2026-07-03 | Completed child goal 6, `assura-agent-content-activation-source-docs`. Agent-ready onboarding now makes content activation explicit with `--content-template none`, `agent-project`, and `document-project`; generated agent projects activate baseline content models and records; generated document projects add source-document custody metadata and manifest reference validation without reading binary targets as UTF-8. The next executable child goal is `assura-agent-lifecycle-hooks-next-actions`. | `docs/goals/assura-agent-content-activation-source-docs.md`; `src/cli/agent_args.rs`; `src/cli/agent_onboarding.rs`; `src/cli/agent_onboarding_templates.rs`; `src/cli/agent_onboarding_content_templates.rs`; `src/cli/doctor.rs`; `src/cli/doctor_content.rs`; `tests/project_intelligence_onboarding.rs`; `tests/doctor_explain_cli.rs`; independent review agent `019f2624-a975-7812-9bcb-cac759e40e73`; `cargo fmt --check`; `cargo test content_runtime --quiet`; `cargo test --test project_intelligence_onboarding --quiet`; `cargo test --test doctor_explain_cli --quiet`; `cargo test --test content_runtime_references --quiet`; `cargo run --quiet -- check --format json .`; `cargo xtask target-state`; `cargo xtask docs`; `cargo xtask evidence`; `git diff --check`. |
| 2026-07-03 | Completed child goal 5, `assura-agent-search-reference-discovery`. Agent-ready projects now have explicit raw/fallback raw text search, configured frontmatter repository-reference extraction, all/unresolved reference listing, context-pack and doctor visibility for frontmatter references, and machine-readable agent-query capability/gap/next-action discovery. The next executable child goal is `assura-agent-content-activation-source-docs`. | `docs/goals/assura-agent-search-reference-discovery.md`; `src/cli/content_query/raw_search.rs`; `src/repository_references.rs`; `src/cli/content_query/agent_query.rs`; `tests/content_query_cli.rs`; `tests/repository_reference_graph_tests.rs`; `tests/repository_reference_check_tests.rs`; `tests/project_intelligence_context_pack.rs`; `tests/doctor_explain_cli.rs`; independent review agent `019f25b7-2361-75b3-b991-66c041ad6b6d`; `cargo fmt --check`; `cargo clippy --workspace --all-targets --all-features -- -D warnings`; `cargo run --quiet -- check --format json .`; `cargo xtask target-state`; `cargo xtask docs`; `cargo xtask evidence`; `git diff --check`. |
| 2026-07-03 | Refined child goal 4 after follow-up guidance on progressive disclosure. Agent-ready onboarding now emits an AGENTS.md use-case routing table, a best-practices reference, and SKILL.md reference-section checks so agents can see when to load project-local skills and where each skill points for deeper docs. | `docs/goals/assura-agent-guidance-skill-contracts.md`; `src/cli/check/agent_guidance.rs`; `src/cli/check/agent_guidance/routing.rs`; `src/cli/check/agent_guidance/skill.rs`; `tests/agents_md.rs`; `tests/skill_contract.rs`; `crates/assura-check-cli/tests/compiled_agent_guidance_cli.rs`; `cargo fmt --check`; `cargo clippy --workspace --all-targets --all-features -- -D warnings`; `cargo run --quiet -- check --format json .`; `cargo xtask docs`; `cargo xtask evidence`. |
...
script-backed computed checks. Work through the linked child goals in order and
keep one-off project-type behavior out of the core presets.
