...
| 2026-07-01 | Started Epic 9, VS Code, with the first experimental extension package over shared Assura CLI JSON contracts. The slice adds diagnostics, daemon health/lifecycle/doctor/logs commands, safe-fix preview-only behavior, structure allowlisting, and experimental support classification; independent review Leibniz's non-zero JSON blocker was fixed before commit. Epic 9 remains open for deeper editor-session/daemon-backed diagnostics and closure review. | [assura-vscode-daemon-integration.md](./assura-vscode-daemon-integration.md); `integrations/editors/vscode/package.json`; `integrations/editors/vscode/src/assura-client.js`; `integrations/editors/vscode/src/extension.js`; `integrations/editors/vscode/tests/assura-client.test.js`; `.assura/config.yml`; independent review Leibniz; `pnpm test`; `pnpm run build`; `cargo run --quiet -- check --format json .`; `cargo xtask target-state`; `cargo xtask evidence`; `cargo xtask docs`; `git diff --check`. |
| 2026-07-01 | Continued Epic 9 with daemon/editor-backed changed-document diagnostics. The VS Code adapter now uses `daemon check-path` plus `editor session` for saved and active workspace files, keeps broad check output as fallback, preserves LSP ranges/severity, and filters non-workspace documents before invoking Assura; independent review Helmholtz's event-scope blocker was fixed before commit. | [assura-vscode-daemon-integration.md](./assura-vscode-daemon-integration.md); `integrations/editors/vscode/src/assura-client.js`; `integrations/editors/vscode/src/extension.js`; `integrations/editors/vscode/tests/assura-client.test.js`; independent review Helmholtz; `pnpm test`; `pnpm run build`; `cargo run --quiet -- check --format json .`; `cargo xtask target-state`; `cargo xtask evidence`; `cargo xtask docs`; `git diff --check`. |
| 2026-07-01 | Completed Epic 9, VS Code. Independent closure review Darwin found no blocker or high-risk gaps; the extension now meets the beta bar for daemon status/lifecycle commands, changed-document daemon/editor diagnostics, one-shot fallback diagnostics, preview-only safe fixes, package test/build commands, and experimental support classification. Next default epic is the final LS-Lint no-slower gate reassessment and beta completion audit. | [assura-vscode-daemon-integration.md](./assura-vscode-daemon-integration.md); independent closure review Darwin; `pnpm test`; `pnpm run build`; `cargo test --test daemon_cli_tests --quiet`; `cargo test --test editor_surface_cli --quiet`; `cargo run --quiet -- check --format json .`; `cargo xtask target-state`; `cargo xtask evidence`; `cargo xtask docs`; `git diff --check`. |
| 2026-07-01 | Completed Epic 10, LS-Lint Gate, and ran the beta completion audit. Independent review Feynman found the status split correct but required fresh performance evidence; after the lightweight repository-reference build fix, a regenerated clean 5-iteration release report at commit `244f1e5` shows cold `assura-cli` no slower than native `ls-lint-cli` on all 8 headline fixtures. The master beta program remains not complete because `cargo xtask release-readiness --format json` fails while latest GitHub release `v0.1.0` already exists and this branch has unreleased supported/experimental user-facing surfaces; the remaining beta blocker is cutting a new beta/release-candidate tag and artifact with validation evidence. | [assura-ls-lint-no-slower-performance-gate.md](./assura-ls-lint-no-slower-performance-gate.md); independent review Feynman; `cargo build --release --bin assura --no-default-features --features json-output,yaml-config`; `cargo build --release --bin assura-full`; `cargo build --release -p assura-check-cli`; `target/release/assura performance-report --output benches/history/current.json --history benches/history/ls-lint-comparison-history.jsonl --website-dir website/public/data/performance --iterations 5`; `cargo xtask performance-no-slower`; `jq '.commit_sha,.source_worktree_dirty,.claim_summary,.warm_claim_summary,.ls_lint_status' benches/history/current.json`; `cargo xtask release-readiness --format json` (expected fail until release tag/artifact). |
| 2026-07-01 | Prepared the `v0.2.0` beta release-candidate metadata after the ten roadmap epics. Package versions, release notes, release checklist examples, release-surface first-release values, and Assura manifest semantics now agree on `0.2.0`; `cargo xtask release-readiness --format json` passes and local `cargo xtask release-smoke` installs an archive whose binary reports version `0.2.0`. Independent review Gauss found and verified fixes for release-surface validation gaps. The master beta program still remains incomplete until `v0.2.0` is tagged, GitHub release artifacts are published, and live release evidence is recorded. | `Cargo.toml`; `crates/assura-check-cli/Cargo.toml`; `crates/assura-stable-hash/Cargo.toml`; `xtask/Cargo.toml`; `docs/data/release-surfaces.json`; `docs/release-notes.md`; `cargo xtask release-readiness --format json`; `cargo xtask release-smoke`; `cargo test -p xtask release_readiness --quiet`; `cargo run --quiet -- check --format json .`; `cargo xtask target-state`; `cargo xtask evidence`; `cargo xtask docs`; independent review Gauss. |
| 2026-07-01 | Closed the PR #112 release-candidate CI cleanup loop after fixing Windows daemon-core path assertions and preserving the no-slower performance gate. The PR check rollup is green across Rust CI, Documentation, Security Audit, and GitGuardian on commit `6640d64`; the master beta program still remains incomplete until the branch is merged and `v0.2.0` is tagged/published with live release evidence. | PR #112; Rust CI run `28524602822`; Documentation run `28524602655`; Security Audit run `28524602646`; `cargo test --test daemon_core_tests --quiet`; `cargo test --test daemon_cli_tests --quiet`; `cargo fmt --check`; `cargo run --quiet -- check --format json .`; `git diff --check`; `cargo clippy --all-targets --all-features -- -D warnings`; `gh pr view 112 --json statusCheckRollup`. |
| 2026-07-01 | Completed the beta master program after PR #112 merged and `v0.2.0` was tagged and published. The final release evidence closes the prior blocker: `origin/master` points at merge commit `1ea9f95138e38fef9283687a0fc7e2256adfc23d`, remote tag `v0.2.0` peels to the same commit, the GitHub release is published with the five expected platform archives and checksum sidecars, the live public URL gate passes, and release readiness now reports ready. Independent completion review Bernoulli's release blockers are resolved. | PR #112 merged at `2026-07-01T15:17:20Z`; `git ls-remote --tags origin 'refs/tags/v0.2.0*'`; release workflow run `28528132216`; [Release v0.2.0](https://github.com/rothnic/assura/releases/tag/v0.2.0) published at `2026-07-01T15:24:36Z`; `gh release view v0.2.0 --json tagName,isDraft,isPrerelease,publishedAt,url,name,assets`; `ASSURA_VERSION=v0.2.0 cargo xtask release-live`; `cargo xtask release-readiness --format json`; independent review Bernoulli. |
