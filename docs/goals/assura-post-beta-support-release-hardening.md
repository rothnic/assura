...
| 2026-07-02 | Completed the support-hardening child after PR #136 merged into `master`. GitHub CI passed all required release-hardening checks, including the strict Performance Report no-slower gate, platform test suites, installable adoption smokes, release bundle smoke, Windows installer smoke, docs, evidence, clippy, rustfmt, coverage, and security checks. The remaining parent boundary is the explicit `v0.3.0` tag, release workflow, and live release verification step. | PR #136; merge commit `a37d6245d9340237e8cf95452326fc8d4d1f5703`; `gh pr checks 136 --watch --fail-fast`; `cargo xtask release-readiness --format json`; `.trellis/tasks/archive/2026-07/07-02-post-beta-support-release-hardening`. |
| 2026-07-02 | Recorded live `v0.3.0` release evidence after the parent finalization merge and tag. The release workflow succeeded, GitHub published all expected platform archives and `.sha256` files, and both latest and explicit-version live gates reached every expected asset. Assura remains pre-1.0 beta. | Release workflow `28601020713`; `https://github.com/rothnic/assura/releases/tag/v0.3.0`; `gh release view v0.3.0 --json tagName,url,isDraft,isPrerelease,publishedAt,assets`; `cargo xtask release-live`; `ASSURA_VERSION=v0.3.0 cargo xtask release-live`. |
