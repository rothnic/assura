...
| 2026-07-02 | Started parent finalization from merged `origin/master`, archived the completed support-hardening task, and proved the local `v0.3.0` candidate is ready for the explicit tag/live-release step. The parent remains planned rather than completed until live release assets and checksums are published and verified. | `.trellis/tasks/archive/2026-07/07-02-post-beta-support-release-hardening`; `.trellis/tasks/07-02-post-beta-program-finalization/prd.md`; `cargo xtask release-readiness --format json`; `cargo xtask release-smoke`; `cargo xtask target-state`; `cargo run --quiet -- check --format json .`; `cargo xtask docs`; `cargo xtask evidence`; `git diff --check`. |
