...
This parent goal is complete for the `v0.3.0` beta increment. It remains
pre-1.0 beta work, not a GA or post-beta stability claim.

| Criterion | Outcome |
| --- | --- |
| Self-config and staged validation | Completed. Assura's own config and docs variance checks run structure and coarse file policy before deeper Markdown/content/reference work. |
| Supported document graph | Completed for the beta increment. The supported path covers content validation, collection queries, lexical search, bounded graph expansion, repository-reference queries, missing relations, context packs, and local sessions; optional semantic/code-symbol enrichment remains experimental. |
| True daemon mode | Completed for the beta increment. A real local daemon process exposes versioned IPC for health, checks, changed paths, and repository-reference context, with stale-config safety and one-shot fallback. |
| Markdown lint/fix engine | Completed for the beta increment. Native Assura Markdown validation and deterministic safe fixes remain the supported default; `rumdl` is retained as an opt-in markdownlint-compatible adapter because measured evidence showed it was functionally strongest but slower on the small fixture. |
| Performance floor | Completed. Accepted LS-Lint-equivalent fixture rows are enforced row by row, and current checked release evidence shows no accepted realistic-equivalent row slower than native LS-Lint. |
| Agent lifecycle | Completed. Codex, OpenCode, Claude, and Pi local integration bundles have install/update/remove/status/doctor lifecycle commands over shared Assura nudge/check/daemon contracts. |
| VS Code support | Completed as a supported beta local package over shared CLI, daemon, and editor-session contracts, not as a marketplace or full LSP claim. |
| Extension API clarity | Completed. `extensions.*` is documented as first-party config policy; supported integration routes are local JSON contracts; public plugin APIs remain roadmap-only. |
| Release hardening | Completed. `v0.3.0` is tagged, published, and live-verified with all expected archives and checksums reachable. |
| North-star use case | Completed as the acceptance lens for this beta increment. The verification story now ties each child outcome to the maintainer's branch-safety decision and remains the package to expand in future increments. |
...
| 2026-07-02 | Started the Support Hardening child after PR #135 merged. The release-hardening branch prepares `v0.3.0` as the next beta increment by aligning package versions, release notes, release surfaces, target-state checks, and release-readiness evidence while preserving the beta/pre-1.0 support boundary. | [Post-beta support and release hardening](./assura-post-beta-support-release-hardening.md); `.trellis/tasks/07-02-post-beta-support-release-hardening/prd.md`; `cargo xtask release-readiness --format json`; `cargo xtask target-state`. |
| 2026-07-02 | Continued Support Hardening by adding a child-level release verification use-case picture and proving the prepared beta increment through local release smoke. At that point the parent was intentionally left incomplete until support-hardening review/merge and explicit `v0.3.0` release publication evidence existed. | [Post-beta support and release hardening](./assura-post-beta-support-release-hardening.md#release-verification-use-case-picture); `cargo xtask release-smoke`; `cargo xtask release-readiness --format json`; `cargo xtask target-state`; `cargo xtask evidence`. |
| 2026-07-02 | Processed independent review for Support Hardening and tightened the release checklist/target-state contract. The `v0.3.0` candidate now has explicit gates for daemon lifecycle, agent integration lifecycle, VS Code local package smoke, and extension-boundary documentation before any release PR can claim readiness. | Review agent `019f231f-8703-7120-a15c-44553b6eb3a3`; [Post-beta support and release hardening](./assura-post-beta-support-release-hardening.md); `docs/release-candidate-checklist.md`; `xtask/src/main.rs`; `cargo xtask target-state`; focused daemon, agent, and VS Code package gates. |
| 2026-07-02 | Completed the Support Hardening child after PR #136 merged into `master` with all GitHub checks green, including the strict Performance Report no-slower gate. The parent is now in finalization: release readiness is green for local `0.3.0`, but completion still requires the explicit `v0.3.0` tag, release workflow assets, live release verification, and a final completion audit against the north-star scenario. | [Post-beta support and release hardening](./assura-post-beta-support-release-hardening.md); PR #136; merge commit `a37d6245d9340237e8cf95452326fc8d4d1f5703`; `cargo xtask release-readiness --format json`; `.trellis/tasks/07-02-post-beta-program-finalization/prd.md`. |
| 2026-07-02 | Started parent finalization from merged `origin/master`, archived the completed support-hardening task, and proved the local `v0.3.0` candidate was ready for the explicit tag/live-release step. Completion was intentionally deferred until live release assets and checksums were published and verified. | `.trellis/tasks/archive/2026-07/07-02-post-beta-support-release-hardening`; `.trellis/tasks/07-02-post-beta-program-finalization/prd.md`; `cargo xtask release-readiness --format json`; `cargo xtask release-smoke`; `cargo xtask target-state`; `cargo run --quiet -- check --format json .`; `cargo xtask docs`; `cargo xtask evidence`; `git diff --check`. |
| 2026-07-02 | Completed the parent post-beta capabilities program for the `v0.3.0` beta increment. The tag `v0.3.0` was published from merged `master`, the release workflow produced all five platform archives plus checksums, both `latest` and explicit `v0.3.0` live-release gates returned HTTP 200 for every expected asset, and the completion audit maps each requested capability to a beta-supported or explicitly experimental surface. | Release workflow `28601020713`; release URL `https://github.com/rothnic/assura/releases/tag/v0.3.0`; `gh release view v0.3.0 --json tagName,url,isDraft,isPrerelease,publishedAt,assets`; `cargo xtask release-live`; `ASSURA_VERSION=v0.3.0 cargo xtask release-live`; [Completion Audit](#completion-audit). |
