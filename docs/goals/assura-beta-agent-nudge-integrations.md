...
| 2026-07-01 | Added concrete local integration recipes for Codex, OpenCode, Claude, and Pi over the shared nudge payload. The docs now define session-start, before-tool, after-tool, deep diagnostics, and daemon recovery commands without adding per-agent validation paths; the CLI smoke proves all beta agent labels are accepted while non-Codex hosts keep the generic `assura check --format agent` fallback. Closure review remains before Epic 8 can be marked complete. | `integrations/agents/README.md`; `integrations/agents/codex/README.md`; `integrations/agents/opencode/README.md`; `website/src/content/docs/reference/agent-feedback.md`; `tests/agent_surface_cli.rs`; `cargo fmt`; `cargo test --test agent_surface_cli --quiet`. |
| 2026-07-01 | Completed Epic 8, Agent Nudges. Independent closure review Faraday found no blocker or high-risk gaps; follow-up hardening tightened broad inspect-event wording and expanded the CLI smoke to cover Codex, OpenCode, Claude, and Pi across session-start, before-tool, and after-tool events without private validation paths. Next default epic is VS Code daemon integration. | Independent closure review Faraday; `tests/agent_surface_cli.rs`; `integrations/agents/README.md`; `integrations/agents/codex/README.md`; `integrations/agents/opencode/README.md`; `website/src/content/docs/reference/agent-feedback.md`; `cargo fmt --check`; `cargo test --test agent_surface_cli --quiet`; `cargo run --quiet -- check --format json .`; `cargo xtask target-state`; `cargo xtask evidence`; `cargo xtask docs`; `git diff --check`. |
