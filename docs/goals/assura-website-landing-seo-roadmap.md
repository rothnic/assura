...
| 2026-07-10 | Iteration 4: implemented the local Website P2 conversion, examples, methodology, dogfood, and changelog surfaces. All 13 marketing routes have unique metadata and social assets, pass mobile accessibility checks, and have no broken internal links. GitHub repository metadata is aligned and the personal-site Assura case study is open separately. Publication, webmaster discovery, and the profile-pin decision remain open. | `docs/analysis/2026-07-10-website-p2-evidence.md`; 23 Playwright tests; personal-site pull request 60 |
| 2026-07-10 | Iteration 5: established the first monthly Website P3 scorecard with an owner, next review date, complete metric inventory, live HTTP/search baseline, and dated actions. Webmaster fields remain explicitly unavailable until account setup. | `docs/analysis/2026-07-website-search-performance-scorecard.md` |
| 2026-07-10 | Iteration 6: completed Product P1 review/onboarding semantics and Product P2/P3 warm-loop foundations. Review is advisory and versioned, finding history is stable and bounded, cache namespaces and fallback are observable, clean worktrees share immutable results, repeated agent messages have cooldowns, and five release-binary p95 budget rows are CI-gated. | `docs/analysis/2026-07-10-website-product-p1-p3-evidence.md`; `benches/history/warm-loop-current.json`; Product P1 focused suites |
| 2026-07-10 | Iteration 7: closed local implementation and quality proof. The full Rust workspace, target-state and evidence audits, all five warm-loop p95 budgets, the 52-page static build, Assura self-validation, and 23 browser checks pass. Production merge, webmaster submissions, and the GitHub profile-pin decision remain account-owner publication work. | `cargo test --workspace --all-features`; `cargo xtask target-state`; `cargo xtask evidence`; `cargo xtask docs`; `cargo xtask warm-loop-no-regression benches/history/warm-loop-current.json`; `cargo run --quiet -- check --format json .`; `pnpm --dir website test:marketing` |
