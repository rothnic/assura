...
| 2026-06-30 | Added Markdown per-rule severity overrides and reasoned `assura-ignore` suppressions. Supported Markdown rules can now be downgraded or promoted with `markdown.rule_severity`, valid suppressions require `<!-- assura-ignore <markdown_rule>: <reason> -->`, and invalid suppressions report `markdown_suppression`. Missing-heading safe fixes, broad markdownlint compatibility, and outbound link fact ingestion remain open. | `src/cli/check/markdown/suppression.rs`; `tests/markdown_suppression_severity_tests.rs`; `crates/assura-check-cli/tests/compiled_markdown_cli.rs`; `docs/data/release-surfaces.json`; `cargo test --test markdown_suppression_severity_tests --quiet`; `cargo test -p assura-check-cli --test compiled_markdown_cli --quiet`; independent review Jason. |
