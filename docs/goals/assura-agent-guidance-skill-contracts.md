...
| --- | --- | --- |
| 2026-07-03 | Refined child goal 4 after follow-up guidance. `extensions.agent_guidance` can now require an AGENTS.md progressive-disclosure reference, validate an optional use-case-oriented skill routing table, allow configured skill-name patterns, and require configured SKILL.md sections to point to deeper references. Line-limit diagnostics now tell agents to keep AGENTS.md as the router and SKILL.md as a concise index. | `src/cli/check/agent_guidance.rs`; `src/cli/check/agent_guidance/routing.rs`; `src/cli/check/agent_guidance/skill.rs`; `src/config/config/extensions/agent_guidance.rs`; `src/cli/agent_onboarding_templates.rs`; `tests/agents_md.rs`; `tests/skill_contract.rs`; `crates/assura-check-cli/tests/compiled_agent_guidance_cli.rs`; `cargo fmt --check`; `cargo test --test agents_md --quiet`; `cargo test --test skill_contract --quiet`; `cargo test -p assura-check-cli --test compiled_agent_guidance_cli --quiet`; `cargo test --test project_intelligence_onboarding agent_onboard_generated_config_validates_dynamic_directory_skill_contracts --quiet`; `cargo clippy --workspace --all-targets --all-features -- -D warnings`; `cargo run --quiet -- check --format json .`; `cargo xtask target-state`; `cargo xtask docs`; `cargo xtask evidence`. |
