...
| Documentation architecture | There is extensive dated analysis and goal history. This is useful, but the amount of active docs makes stale claims likely unless checks stay strict. A first reusable docs-lifecycle detector slice is in review. | Partially aligned. | Merge the active/docs-lifecycle first slice, then broaden configured active/archive and stale-claim coverage only for deterministic claims. |
...
| Assura-rule generalization | First reusable slices exist for command surface, release contract, public surface support matrix, Cargo manifest semantics, test relationship, and module topology. Docs lifecycle/stale-claim detection has an active first slice pending review/merge. | Good backlog, not done. | Review and merge the docs lifecycle/stale-claim first slice before broad cleanup; then reassess P2 external tooling. |
...
| Dense active docs/history | partially aligned | Many active dated analysis files and release/performance claims; first-slice docs lifecycle policy is in review. | Merge the first-slice detector, then add coverage for concrete stale claim classes before broad cleanup. |
...
| P1 | Broaden docs lifecycle checks beyond the active first slice only for concrete active/archive and stale roadmap/performance claim classes. | `extensions.docs_lifecycles` plus target-state/evidence gates. |
...
| P1 | Active docs can accumulate stale roadmap or historical analysis claims. | `docs/analysis/**`, `docs/goals/**`, website docs | Active docs have lifecycle status and stale public claims are rejected. | Merge the first `extensions.docs_lifecycles` slice, then broaden deterministic claim patterns and historical exceptions. | `cargo xtask evidence` plus `extensions.docs_lifecycles` self-check | Docs lifecycle rule |
