...
| Documentation architecture | There is extensive dated analysis and goal history. This is useful, but the amount of active docs makes stale claims likely unless checks stay strict. A first reusable docs-lifecycle detector slice is merged and dogfood expansion is active for explicit current docs and deterministic claim tokens. | Partially aligned. | Finish and review `docs/goals/assura-rule-docs-lifecycle-dogfood-expansion.md`; keep website docs and `2x` deferred until exact evidence boundaries are available. |
...
| Dense active docs/history | partially aligned | Many active dated analysis files and release/performance claims; first-slice docs lifecycle policy is merged and dogfooded narrowly. | Implement the docs lifecycle dogfood expansion goal before broad cleanup. |
...
| P1 | Implement `docs/goals/assura-rule-docs-lifecycle-dogfood-expansion.md` for concrete active/archive and stale roadmap/performance claim classes. | `extensions.docs_lifecycles` plus target-state/evidence gates. |
...
| P1 | Active docs can accumulate stale roadmap or historical analysis claims. | `docs/analysis/**`, `docs/goals/**`, website docs | Active docs have lifecycle status and stale public claims are rejected. | Implement the dogfood expansion goal with bounded active docs, deterministic claim patterns, and historical exceptions. | `cargo xtask evidence` plus `extensions.docs_lifecycles` self-check | Docs lifecycle rule |
