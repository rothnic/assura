...
| Documentation architecture | There is extensive dated analysis and goal history. This is useful, but the amount of active docs makes stale claims likely unless checks stay strict. A first reusable docs-lifecycle detector slice is merged and dogfooded narrowly. | Partially aligned. | Implement `docs/goals/assura-rule-docs-lifecycle-dogfood-expansion.md` to broaden configured active/archive and stale-claim coverage only for deterministic claims. |
