...
| Dense active docs/history | partially aligned | Many active dated analysis files and release/performance claims; first-slice docs lifecycle policy is merged and dogfooded narrowly. | Add coverage for concrete stale claim classes before broad cleanup. |
...
| P1 | Broaden docs lifecycle checks beyond the completed first slice only for concrete active/archive and stale roadmap/performance claim classes. | `extensions.docs_lifecycles` plus target-state/evidence gates. |
...
| P1 | Active docs can accumulate stale roadmap or historical analysis claims. | `docs/analysis/**`, `docs/goals/**`, website docs | Active docs have lifecycle status and stale public claims are rejected. | Broaden deterministic claim patterns and historical exceptions beyond the completed first slice. | `cargo xtask evidence` plus `extensions.docs_lifecycles` self-check | Docs lifecycle rule |
