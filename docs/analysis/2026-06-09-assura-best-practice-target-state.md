...
| Assura-rule generalization | First reusable slices exist for command surface, release contract, public surface support matrix, Cargo manifest semantics, test relationship, and module topology. Docs lifecycle/stale-claim detection remains the highest-risk unimplemented detector. | Good backlog, not done. | Prioritize docs lifecycle/stale-claim detection before broad cleanup; then reassess P2 external tooling. |
