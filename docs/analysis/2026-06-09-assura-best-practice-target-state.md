...
| Module boundaries | `src/` is broad and about 43k lines. Several files cluster just under 500 lines, which suggests current limits are shaping files but not proving cohesion. Experimental `intelligence`, `maturity`, and broad `validation` APIs are contained and now have first-slice support-matrix and module-topology dogfood policy. | Partially aligned/contained. | Use module-topology reports before deleting or splitting; expand topology coverage only where it prevents concrete drift. |
...
| Documentation architecture | There is extensive dated analysis and goal history. This is useful, but the amount of active docs makes stale claims likely unless checks stay strict. A first reusable docs-lifecycle detector slice is merged and dogfood coverage now includes explicit current docs plus deterministic release and `performance-report` claim tokens. | Aligned for selected current docs; deferred for website docs and `2x`. | Add more docs lifecycle coverage only when a bounded goal names explicit website docs or exact `2x` evidence/token boundaries. |
...
| Release/versioning | Release docs, support policy, installers, workflow gates, manifest semantics, and first-slice release-contract checks exist. | Partially aligned. | Keep release-sync checks active and broaden only when concrete release drift appears. |
...
| Assura-rule generalization | First reusable slices exist for command surface, release contract, public surface support matrix, Cargo manifest semantics, test relationship, module topology, and docs lifecycle/stale-claim detection. | Good backlog, not done. | Reassess concrete detector gaps before broad cleanup; prioritize follow-up coverage only where current evidence exposes deterministic drift. |
...
| Dense active docs/history | partially aligned | Many active dated analysis files remain, but current release/support/validation/roadmap docs now have explicit docs lifecycle dogfood coverage. | Defer website docs and `2x` coverage until exact evidence boundaries are available. |
...
| P1 | Add any further docs lifecycle coverage only for explicit website docs or exact `2x` evidence/token boundaries. | Completed docs lifecycle dogfood expansion plus target-state/evidence gates. |
| P1 | Broaden release-sync checks only when current release docs, installers, support docs, website install copy, or workflows expose a concrete uncovered drift case. | Completed release-contract rule plus evidence verifier. |
...
| P1 | Website docs or `2x` performance claims can drift if they are later added without exact evidence boundaries. | Website docs, performance JSON, performance reference docs | Additional docs lifecycle coverage is explicit, token-executable, and backed by committed evidence. | Create a bounded follow-up only after exact evidence/token boundaries are defined. | `cargo xtask evidence` plus `extensions.docs_lifecycles` self-check | Docs lifecycle rule |
| P1 | Module topology limits cap file size but do not prove cohesion everywhere. | `src/cli/check/**`, `src/cli/performance_report/**`, `src/{intelligence,maturity,validation}` | Module families have named ownership and public-support status where exposed or drift-prone. | Use completed module topology rule before large refactors; add rows only for concrete drift-prone families. | `extensions.module_topologies` self-check | Human architecture review |
