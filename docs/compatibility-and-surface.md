...
| `config:extensions.computed_checks` | Experimental first-party | Computed-check tests prove opt-in project-local scripts emit versioned JSON findings, handle missing/invalid/timeout/nonzero behavior, appear in doctor and agent-query gaps, and round-trip through compiled check artifacts without becoming a public plugin API. |
