...
| `assura agent onboard` | Experimental local agent-ready onboarding surface | Project-intelligence onboarding tests prove the command generates a broad baseline, onboarding packet, checked/unchecked report, reusable dynamic skill-directory contract, explicit nudge/warn/gate lifecycle profiles, ranked next actions, optional `--content-template agent-project`, `document-project`, and `proposal-sbir` activation, source-document custody metadata, generic document-project topics/drafts/final-doc records, explicit proposal/SBIR domain-pack records/checks, and non-destructive existing-file behavior. |
