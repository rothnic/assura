...
| `assura init` | Supported | Installable adoption smoke; `tests/agent_recipe_file.rs` proves explicit local `--recipe-file` application, invalid-policy refusal, provenance, and idempotence. |
...
| `assura agent onboard` | Supported local agent-ready onboarding surface | Project-intelligence onboarding tests prove the command generates a broad baseline, project-local maintenance and structure-fit skills, onboarding packet, checked/unchecked report, reusable dynamic skill-directory contract, explicit nudge/warn/gate lifecycle profiles, ranked next actions, optional `--content-template agent-project` and `document-project` activation, source-document custody metadata, generic document-project topics/drafts/final-doc records, and non-destructive existing-file behavior. Explicit `--recipe-file` is local-only, validated, provenance-recorded, and preserves collisions without a partial config write. |
