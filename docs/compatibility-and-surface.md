...
| `assura agent onboard` | Experimental local agent-ready onboarding surface | Project-intelligence onboarding tests prove the command generates a broad baseline, onboarding packet, checked/unchecked report, reusable dynamic skill-directory contract, optional `--content-template agent-project` and `document-project` activation, source-document custody metadata, and non-destructive existing-file behavior. |
