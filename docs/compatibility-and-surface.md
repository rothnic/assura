...
| `config:extensions.agent_guidance` | Experimental first-party | Agent guidance contract tests prove opt-in `AGENTS.md` and project-local `SKILL.md` checks for required sections, frontmatter fields, skill index links, use-case skill routing tables, progressive-disclosure references, supporting reference links, and concise entrypoints. |
