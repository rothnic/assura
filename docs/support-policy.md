...
| Repository reference facts | Experimental project-intelligence graph | Conservative Markdown, comment, docstring, and string-literal path references may appear as `RepositoryReference` edges in graph-oriented content surfaces. Lower-confidence source-derived edges are inspection context until full check-report integration is complete. |
