...
| Repository reference facts and checks | Experimental project-intelligence graph | Conservative Markdown, comment, docstring, and string-literal path references may appear as `RepositoryReference` edges in graph-oriented content surfaces. Opt-in `extensions.repository_references` checks may report locally provable missing targets, missing Markdown anchors, and invalid line anchors. |
...
| `assura content` collection validation and query commands | Supported first project-intelligence query surface | JSON output for collection, instance, show, relation, keyword, bounded graph, context-pack, agent-context, agent-query, and JSON-line session queries must remain deterministic enough for agent use. These commands are backed by content runtime models and the shared Project Intelligence fact store. Experimental repository-reference graph queries are available through the same command group. |
...
| `assura daemon` | Experimental local daemon management preview | Status, doctor, health, changed-path, and reference-context commands expose local JSON contracts over daemon-ready state. Full start/stop/restart/logs process lifecycle support remains roadmap until implemented and tested. |
