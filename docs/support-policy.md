...
| `extensions.agent_guidance` | Experimental first-party | Configured `AGENTS.md` and project-local `SKILL.md` routing checks for required sections, use-case skill routing tables, progressive-disclosure references, skill index links, frontmatter fields, stable headings, supporting reference links, and concise entrypoints. It does not provide a global skill registry or host-agent-specific validation engine. |
