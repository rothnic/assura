...
| `assura review` | Experimental compact project review | Read-only first diagnostic command over existing check, doctor, and content-query truth. Text output must stay compact, JSON uses `assura.project-review.v1`, agent output uses `assura.project-review.agent.v1`, and raw generated/archive/log/benchmark reference noise must not become blocking policy by itself. |
