...
| `assura content` collection validation and query commands | Supported first project-intelligence query surface | JSON output for collection, instance, show, relation, keyword, bounded graph, context-pack, agent-context, agent-query, and JSON-line session queries must remain deterministic enough for agent use. These commands are backed by content runtime models and the shared Project Intelligence fact store. |
