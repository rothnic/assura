...
| `assura init` | Supported | Starter config output must be valid and self-checkable. `--recipe-file <path>` reads only explicit local YAML, validates the complete prospective policy before writing, and records source/hash provenance separately from policy semantics. |
...
| `markdown.lint_common`, `markdown.check_links`, and Markdown severity/suppressions | Supported next-release deterministic Markdown validation | These checks stay local and offline, report stable `markdown_*` rule IDs, validate locally provable links and anchors, and remain suppressible/configurable through the shared Markdown severity contract. Safe-fix apply remains a separate experimental surface. |
...
| `assura agent onboard` | Supported next-release agent-ready onboarding surface | Creates or preserves a broad non-domain-specific baseline, optionally activates `--content-template agent-project` or `document-project`, generates `.assura/onboarding/`, installs project-local maintenance and structure-fit skills under `.agents/skills/`, runs local verification, and tells agents which specialization questions to ask next. With an explicit local `--recipe-file <path>`, it preflights an existing policy before baseline materialization, atomically applies only a validated conflict-free local fragment, and records source/hash provenance. With an explicit concrete host and `--activate`, onboarding also installs and activates the project-local Codex, OpenCode, Claude, or Pi integration. The generated baseline uses reusable dynamic skill-directory contracts instead of enumerating every project-local skill. It must not implement remote bootstrap behavior, silently overwrite user-authored files, mutate host configuration without explicit activation, or add domain-specific behavior to generic presets. |
| `assura agent nudge` | Supported next-release local agent event payload | Event-aware nudges for Codex, OpenCode, Claude, and Pi wrappers stay bounded, avoid volatile default fields, reuse `assura check --format agent` and daemon contracts, suppress identical messages during the configured cooldown, and never introduce per-agent validation logic. |
| `assura agent integration` | Supported next-release managed project-local lifecycle | Install, activate, update, deactivate, remove, status, and doctor commands manage Assura-owned integration state for Codex, OpenCode, Claude, and Pi. Activation is explicit, transactional, path-bounded, preserves unmanaged host configuration, and delegates validation to shared nudge/check/daemon commands. |
...
| `extensions.agent_guidance` | Supported next-release deterministic policy | Configured `AGENTS.md` and project-local `SKILL.md` routing checks cover required sections, use-case skill routing tables, allowed skill-name patterns, progressive-disclosure references, skill index links, frontmatter fields, stable headings, SKILL doc-routing tables, supporting reference links, and actionable concise-entrypoint remediation. It does not provide a global skill registry or host-agent-specific validation engine. |
