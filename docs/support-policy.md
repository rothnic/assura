...
| `assura review` | Experimental compact project review | Read-only first diagnostic command over existing check, doctor, content-query, and best-effort Git truth. Text output must stay compact, JSON uses `assura.project-review.v1`, agent output uses `assura.project-review.agent.v1`, `heatmap` remains advisory/non-fatal outside Git checkouts, and raw generated/archive/log/benchmark reference noise must not become blocking policy by itself. |
