...
| `assura daemon` | Experimental local daemon management preview | Status, start, stop, restart, doctor, logs, health, changed-path, and reference-context commands expose local JSON contracts over daemon-ready state and project-local runtime metadata. A long-running socket/process server remains roadmap until implemented and tested. |
