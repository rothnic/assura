...
| `extensions.computed_checks` | Experimental first-party | Opt-in project-local script-backed computed findings with versioned JSON input/output, allowlisted script paths, optional `windows_script` selection, bounded timeouts, and normal Assura diagnostics. Not remote plugin loading, shell command interpolation, a marketplace, or a domain-specific scoring preset. |
