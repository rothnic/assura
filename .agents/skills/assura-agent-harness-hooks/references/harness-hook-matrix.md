...
- Delivery: Codex, OpenCode, Claude, and Pi adapters call shared Rust `--delivery automatic`; automatic output is one bounded line or quiet
- Gaps: host worker survival/trust remains host-owned for adapters without installed-entry proof; actual installed-hook and equivalent LS-Lint performance evidence belongs to CF04
